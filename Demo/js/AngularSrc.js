var app = angular.module("myModule", [ 'angularUtils.directives.dirPagination',
		'ngSanitize', 'ui.bootstrap', 'matchMedia' ]);
app
		.controller(
				"myController",
				function($scope, $http, $timeout, $compile, screenSize) {
					...
					$scope.desktop = screenSize.on('sm, md, lg',
							function(match) {
								$scope.desktop = match;
					...
					$scope.clearResults = function() {
						...
						for (var i = 0; i < $scope.searchResults.length; i++) {
							google.maps.event.clearListeners(
									$scope.searchResults[i].Marker, 'click');
					...
					$scope.getFilteredData = function(searchTerms,
							searchProviders, searchServices) {

						console.log("Requesting Data For Search: Term="
								+ searchTerms + " Services=" + searchServices
					...
					$scope.map2 = new google.maps.Map(document
							.getElementById('map2'), mapOptions2);
					jQuery('#service-info')
							.on(
									'shown.bs.modal',
									function() {

										google.maps.event.trigger($scope.map2,
												'resize', {});
										$scope.map2
												.setCenter(new google.maps.LatLng(
														$scope.activeResults.Lat,
														$scope.activeResults.Long));
										google.maps.event
												.addDomListener(
														window,
														"resize",
					...
					$scope.createGeoMarker = function(sr, onPageLoad) {
						...
						'<h4>'
								...
								+ sr.Address
								+ '</a></div>'
								+ '<div><span class="glyphicon glyphicon-calendar" style="float:left;"></span> '
								+ sr.Hours
								+ '</div>'
								...
								+ '<a href="'
								+ sr.WebSite
								+ '" target="_blank"> Homepage</a></div>'
								+ '<button class="btn btn-default" style="float:right;" data-toggle="modal" data-target="#service-info" ng-click="changeActiveMarker('
								+ sr.Index
								+ ')" data-keyboard="true">More</button>'
								+ '</div>';
						...
						var marker = new google.maps.Marker({
							...
							fit : true,
							content : onPageLoad ? $compile(
									infoWindowContentHtml)($scope)[0] : null
						});
